            ...
            <div class="box-logged btn-block">
                <div>
                    <nav class="navbar navbar-expand-lg navbar-light bg-light justify-content-between">
                        <a class="navbar-brand" href="homeAluno.php"><img src="img/logo.png" class="img-fluid" width="100"></a>
                        <div>
                        ...
                        <div class="bg-light p-4 rounded">
                            ...
                            <?php
                                if(isset($_POST['senha']) && isset($_POST['confirmaSenha'])){
                                    $stmt = $conn->prepare('UPDATE alunos SET senha = :senha WHERE cpf = :cpf');
                                    $stmt->execute(array(
                                        ':cpf'   => $id,
                                        ':senha' => $_POST['senha']
                                    ));
                                    echo "<script>alert('Alteração concluída!');window.location.replace('homeAluno.php');</script>";
                                }
                            ?>
                        </div>
                    </div>
